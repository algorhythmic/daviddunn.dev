...
// nodes (the same effect as the projects-page background) that fades out around
// the button. The button fill is translucent so the single grid shows through it —
// so grid inside and outside the button are always aligned.

const NODE_COLORS = ["#3b82f6", "#ec4899", "#10b981", "#8b5cf6"]
const glow = (c: string) => `0 0 4px ${c}, 0 0 9px ${c}, 0 0 14px ${c}aa`

// Nodes that traverse the field horizontally
const H_NODES = [
  { top: "38%", rev: false, color: NODE_COLORS[0], dur: "6s", delay: "0s" },
  { top: "62%", rev: true, color: NODE_COLORS[1], dur: "7s", delay: "1.4s" },
]
// Nodes that traverse the field vertically
const V_NODES = [
...
// Gradually fade the field out toward its edges
...
export function ProjectsButton() {
  return (
    ...
    >
      ...
      {/* Button face — translucent so the field shows through; dark-mode colors
          match the projects page (slate fill, white text, neo-blue border) */}
